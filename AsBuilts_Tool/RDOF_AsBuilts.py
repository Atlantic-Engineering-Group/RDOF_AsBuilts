...
def splice_sheet(lcp):
    ...
    arcpy.management.AddField(f'{lcpNameFixed}_Adds_Drop_Fiber_Splice', 'Assigned_Ports', "TEXT")
    ...
    arcpy.management.AddField(f'{lcpNameFixed}_Adds_Drop_Fiber_Splice', 'OLT_PONfiber', "TEXT")

    arcpy.TableToExcel_conversion(f'{lcpNameFixed}_Adds_Drop_Fiber_Splice',
    ...
    data = df[
        ['Address', 'City', 'Zip', 'NAP', 'Cable Name', 'Assigned Splitter', 'FiberCount', 'Row 7', 'Assigned_Ports',
         'Assigned Fiber Count', 'OLT_PONfiber']].copy()

    data.columns = ['Address', 'City', 'Zip', 'NAP', 'Fiber Cable', 'Assigned Splitter', 'Assigned Port', 'Row 7',
                    'Assigned_Ports', 'AssignedFiberCount', 'OLT PON fiber']
    ...
    data.loc[:, 'OLT PON fiber'] = ponlabel
    ...
    fe_split['Assigned Splitter'] = fe_split['splitter'] + 'SPL' + fe_split['number']

    merge = data.merge(fe_split, left_on='Assigned Splitter', right_on='Assigned Splitter', how="outer")

    merge.loc[merge['Count'] == merge['Count'].fillna(0, inplace=True)]
    ...
    final = merge[['Address', 'City', 'Zip', 'NAP', 'Fiber Cable', 'Assigned Splitter', 'Assigned Port',
                   'AssignedFiberCount', 'OLT PON fiber', 'Count', 'OLT shelf at cabinet']].copy()
